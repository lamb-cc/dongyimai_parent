...
public class ItemSearchServiceImpl implements ItemSearchService {
    ...
    private Map searchList(Map searchMap) {
        ...
        options.addField("item_title");                        //设置标题高亮显示
    ...
    public void importItem(List<TbItem> itemList) {
        ...
        solrTemplate.commit();
        System.out.println("新增商品导入solr成功");
    }
    ...
    public void deleteByGoodsIds(List ids) {
        ...
        solrTemplate.commit();
        System.out.println("从solr中删除商品成功"+ids);
    }
    ...
    /**
     * 根据商品分类，在redis缓存中查询品牌和规格集合（在运营商管理后台，操作模板模块、分类模块时，将三级分类名称+模板ID、模板ID+品牌列表、模板ID+规格集合 3张表缓存进redis）
     * @param category
     * @return
     */
